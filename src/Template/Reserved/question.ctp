...
<section id="services" class="text-center ">
    <div class="container">
        ...
        <section class="row" id="nothing-found" style='position:relative;'>
        ...
        <div class="row" id="question-header" style='position:relative;'>
            ...
            <div id="banner">
                <div class="col-md-4 col-md-offset-4">
                    <?php if($qk - 1 > 0): ?>
                        <a href='#' class='navi' onClick='window.location.href="<?= $this->Url->build(["action" => "question", $question_list[$qk-2]['id']])?>"'>
                            <i class="fa fa-angle-left" ></i>
                        </a>
                    <?php endif ?>
                    <input type="number" id="selector" value="<?= $qk ?>"> 
                    <span> / </span> 
                    <?= count($question_list) ?> 
                ...
                <?php $tot = $question['a1']+$question['a2']+$question['a3']+$question['a4']+$question['a5']; ?>
                <?php if($tot > 50): ?>
                    <?php $corr = $question['a'.$question['correct']] / $tot; ?>
        ...
        <div class="row" id="question-body" style='position:relative;'>
            <div class="reserved-background"></div>
            <div class="col-md-10 col-md-offset-1">
                <div class='panel-body tab-content'> <b><?= $qk.". ".$question['question']; ?></b>
                    ...
                    <?php if($question['pic'] != ''): ?>
                        <div id=question-image> 
                          <img src='<?="$url/img/questions/$question[pic]"?>'/> 
                        </div>
                    <?php endif; ?>

                    <?php if ($question['op1'] != ''): ?>
                      <input type='radio' name='q' value='1' class='radio-btn' <?= @$question_list[$qk-1]['answer'] == 1 ?  "checked" : "" ?> /> 
                      <span id='l_1'> <?= $question['op1']; ?> </span>
                      <br> 
                    <?php endif ?>
                    <?php if ($question['op2'] != ''): ?>
                      <input type='radio' name='q' value='2' <?= @$question_list[$qk-1]['answer'] == 2 ?  "checked"  : ""?>  /> 
                      <span id='l_2'><?= $question['op2']; ?> </span>
                      <br> 
                    <?php endif ?>
                    <?php if ($question['op3'] != ''): ?>
                      <input type='radio' name='q' value='3' <?= @$question_list[$qk-1]['answer'] == 3 ?  "checked"  : ""?>  /> 
                      <span id='l_3'><?= $question['op3']; ?> </span>
                      <br> 
                    <?php endif ?>
                    <?php if ($question['op4'] != '') :?> 
                      <input type='radio' name='q' value='4' <?= @$question_list[$qk-1]['answer'] == 4 ?  "checked"  : ""?> /> 
                      <span id='l_4'><?= $question['op4']; ?> </span>
                      <br> 
                    <?php endif ?>
                    <?php if ($question['op5'] != ''): ?>
                      <input type='radio' name='q' value='5' <?= @$question_list[$qk-1]['answer'] == 5 ?  "checked"  : ""?> /> 
                      <span id='l_5'><?= $question['op5']; ?> </span>
                      <br> 
                    <?php endif ?>
                    <br> 
                    ...
                    <p style='text-align: center'> 
                        ...
                        <button class='submit btn-black btn' id='b<?= $qk ?>' <?= $question_list[$qk-1]['status'] > 0 ?  "disabled" : "" ?>> Validar 
                        </button> 
